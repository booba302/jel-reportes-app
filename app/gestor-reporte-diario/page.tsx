...
export default function GestorReportesPage() {
  ...
  const [mesFiltro, setMesFiltro] = React.useState<string>(format(new Date(), "yyyy-MM"));
  ...
  const fetchHistorial = async () => {
    ...
    try {
      ...
      data.sort((a, b) => new Date(b.fechaReporte).getTime() - new Date(a.fechaReporte).getTime());
      setReportes(data);
    } catch (error) {
    ...
    <div className="p-6 max-w-5xl mx-auto space-y-8">
      <div className="flex flex-col md:flex-row justify-between md:items-end gap-4">
        ...
        <div className="flex items-center gap-3">
          <label className="text-sm font-medium text-slate-600">Filtrar por Mes:</label>
          <input 
            type="month" 
            value={mesFiltro}
              ...
              {isLoading ? (
                ...
                reportesFiltrados.map((rep) => (
                  <TableRow key={rep.id}>
                    <TableCell className="pl-6 font-semibold text-slate-800">
                      {format(new Date(rep.fechaReporte), "dd 'de' MMMM, yyyy", { locale: es })}
                    </TableCell>
                    ...
                    <TableCell className="text-right pr-6 space-x-2">
                      <Button 
                        variant="outline" 
                        size="sm" 
                        onClick={() => handleVerDashboard(rep.fechaReporte, rep.moneda)}
                ...
                <TableRow>
                  <TableCell colSpan={4} className="h-32 text-center text-slate-500">
                    ...
                    No hay reportes cargados en {currency} para el mes seleccionado.
                  </TableCell>
                </TableRow>
              )}
            </TableBody>
